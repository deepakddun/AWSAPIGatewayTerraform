...
terraform {
  ...
  backend "s3" {
    ...
    key    = "api_gateway/lambda_function/terraform_api_gateway_lambda_creat_presigned_url.tfstate"
    region = "us-east-2"
...
resource "aws_lambda_function" "get_average_rating_lambda" {
  ...
  function_name = "pre_signed_url"
  role          = data.aws_iam_role.lambda_role_name.arn
  handler       = "pre_signed_url.lambda_handler"
