...
terraform {
  ...
  backend "s3" {
    ...
    key    = "api_gateway/lambda_function/terraform_api_gateway_lambda_parallel_2.tfstate"
    region = "us-east-2"
...
resource "aws_lambda_function" "parallel_2_lambda" {
  filename      = "lambda_dependency_and_function.zip"
  function_name = "parallel_2"
  role          = data.aws_iam_role.lambda_role_name.arn
  handler       = "parallel_2.handler"
