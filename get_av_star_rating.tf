...
resource "aws_api_gateway_integration_response" "get_av_rating_integration_response" {
  ...
  depends_on = [aws_api_gateway_integration.get_av_rating_integration]
}

resource "aws_api_gateway_method_response" "get_av_rating_integration_method_response" {
  http_method = "GET"
  resource_id = aws_api_gateway_resource.get_av_rating.id
  rest_api_id = aws_api_gateway_rest_api.first_api.id
  status_code = 200
  ...
  depends_on = [aws_api_gateway_method.get_av_rating_method]
}